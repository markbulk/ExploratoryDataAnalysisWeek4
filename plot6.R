...
ggplot(data=dt.plot,
    ...
    geom_text(mapping=aes(y=baseline/1e3 + 0.2, label = paste0(round(percent_change*100, 1), '%'))) +
    scale_x_discrete(name = "Year") + 
    scale_y_continuous(name = "Vehicle-Related PM2.5\n(Thousands of Tons)") +
    facet_wrap(~ city) +
    geom_text(x = 1, label = "Percentage Change from 1999", mapping=aes(y = baseline/1e3 + 0.6), hjust = 0 )
dev.off()